...
temp = permute(sum(P_dis - P_ch), [2, 3, 1]);% 把DER的功率聚合
temp = reshape(temp, REST_SLOTS + 1, NOFSCEN);

Constraints = [Constraints, repmat(Bid_P, 1, NOFSCEN) + ...
    repmat(Bid_R, 1, NOFSCEN) .* repmat(param.d_s', REST_SLOTS, 1) - temp(2 : end, :) == 0];% 未来各时段
Constraints = [Constraints, repmat(Bid_P_cur, 1, NOFSCEN) + ...
    repmat(Bid_R_cur, 1, NOFSCEN) .* param.d_s' - temp(1, :) == 0];% 当前时段
