...
plot(signals((hour - 1) * 1800 + 1 : hour * 1800));

x1 = xlabel('Time','FontSize',13.5,'FontName', 'Times New Roman','FontWeight','bold');          %轴标题可以用tex解释
y1 = ylabel('Regulation Signal','FontSize',13.5,'FontName', 'Times New Roman','FontWeight','bold');
ax = gca;
ax.YTick = [-1 : 0.1 : 1];
ax.XTick = [0 : 300 : 1800];
...
m=linspace(datenum(hour - 1 + ":00",'HH:MM'),datenum(hour  + ":00",'HH:MM'), 7);
