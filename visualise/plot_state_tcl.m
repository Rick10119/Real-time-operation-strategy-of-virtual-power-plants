%% 空调
%% 找一个状态量来看看情况。电动汽车的
id = 123;linewidth = 1.5;

range = 540:60*17;

load("../results/result_prop_ctrl_sep_21.mat");

plot(28 - result.E_rev(id, range), 'g', 'linewidth', linewidth); hold on

load("../results/result_tx_optimal_bid_ctrl_sep_21.mat");

plot(28 - result.E_rev(id, range), 'b', 'linewidth', linewidth);

load("../results/result_optimal_bid_ctrl_sep_21.mat");

plot(28 - result.E_rev(id, range), 'r', 'linewidth', linewidth); hold off


% 设置
y1 = ylabel('Indoor Temperature (℃)','FontSize',13.5,'FontName', 'Times New Roman','FontWeight','bold');
